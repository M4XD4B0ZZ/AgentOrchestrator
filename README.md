...
Most cases run the same invocation through **both** runners — `runCommand`, the
contract AO has today, and `runOwnedCommand` — and require them to agree about
...
semantics, which a green suite of its own tests cannot see. It has already
caught two: a Windows exit code reported signed on one path and unsigned on the
other, and a byte budget the caller had disabled being enforced anyway.
