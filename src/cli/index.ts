...
const DESCRIPTION = [
  ...
  '    127.0.0.1 that serves a small mobile-first page and the JSON snapshot',
  '    that page polls — what this machine can observe of its own durable state',
  '    — for a browser on this machine. It writes nothing, takes no lease and',
  '    starts no program, and it cannot say whether orchestration is',
  ...
  '    a read-only page and the JSON snapshot behind it to whatever can already',
  '    reach this machine. It accepts a request whose Host header it was told to',
  '    accept and refuses the rest, which is routing hardening and is not',
  '    authentication: this build authenticates nobody, derives no trust from a',
  '    remote address or a forwarding header, and terminates no TLS. Reaching it',
  '    from a phone is an access layer an operator puts in front of it, which',
  '    this build neither provides nor learns about',
  '',
