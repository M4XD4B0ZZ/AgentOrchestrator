...
const DESCRIPTION = [
  ...
  '',
  'Network access, stated in full. Two are opt-in and are the only requests this',
  'build makes on its own account:',
  ...
  '    inside a repository. This process makes that request itself',
  '  - `delivery --observe`: one read-only question to github.com about one exact',
  '    commit, made by the GitHub CLI on this build’s behalf. Without --observe',
  '    the same command contacts no forge',
  '',
  'Attended execution reaches a network too, by starting programs that are',
  'network clients in their own right: `run --attended` and `block --attended`',
  'run the agent CLIs, and a repository profile’s verification commands are',
  'whatever that profile declares. That counts as network access here, under the',
  'same rule the delivery residual L-V4-02-6 applies to the GitHub CLI — a',
  'spawned client’s traffic is the command’s traffic.',
  '',
