...
const DESCRIPTION = [
  ...
  '    `--publish-head --attended` creates the task’s branch on the delivery',
  '    remote at its pinned commit — create-only, and the one thing any command',
  '    in this build can change outside this machine',
  '',
  ...
  '    does not suppress',
  '  - `delivery --publish-head --attended` starts Git up to five times. Two ask',
  '    the local config whether that remote reads and writes the same repository,',
  '    and contact nothing. The other three contact the delivery remote: one reads',
  '    the ref, one creates it at one commit if it was absent, and one reads it',
  '    back. The last runs whatever the push reported, and it is the one that',
  '    decides the answer. Git authenticates with the credential helper the',
  '    machine already has; no token is read, carried or written by this build.',
  '    This is the only request any command here makes that can change anything',
