...
const DESCRIPTION = [
  ...
  '    something outside this machine, and each needs `--attended` of its own:',
  '    `--publish-head`, `--create-pr` and `--merge-pr`',
  '',
  ...
  '    does not suppress',
  '  - `delivery --publish-head --attended` contacts the delivery remote up to',
  ...
  'and only when an operator asks for it in that invocation: `--publish-head`,',
  '`--create-pr` and `--merge-pr`, each with `--attended`, each authorised',
  'separately, and at most one attempted per invocation. `--drive` works out',
