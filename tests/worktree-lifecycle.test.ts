...
async function prepared(repository: ResolvedRepository, taskId: string): Promise<TaskWorkspace> {
  const result = await prepareTaskWorkspace(repository, taskWithId(taskId), { lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
...
describe('I — a clean, owned workspace is released completely', () => {
  ...
  it('is safe to prepare the same task again afterwards', async () => {
    ...
    const again = await prepareTaskWorkspace(repository, taskWithId('V1-03'), { lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
