...
describe('a race lost between the check and the create', () => {
  it('is refused when a competitor creates the branch first', async () => {
    ...
    const result = await prepareTaskWorkspace(repository, taskWithId('V1-03'), { git: runner, lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
...
describe('classifying `merge-base --is-ancestor`', () => {
  ...
  it.each([
    ...
    const repository = await freshRepository();
    const prepared = await prepareTaskWorkspace(repository, taskWithId('V1-03'), { lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
...
describe('Git that cannot answer', () => {
  ...
  it('reports WORKTREE_PARENT_UNUSABLE when the workspace directory cannot exist', async () => {
    ...
    const result = await prepareTaskWorkspace(repository, taskWithId('V1-03'), { lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
  ...
  it('never reads a non-zero exit as a successful answer during removal', async () => {
    const repository = await freshRepository();
    const workspace = await prepareTaskWorkspace(repository, taskWithId('V1-03'), { lease: leaseFor(repository), base: { kind: 'DEFAULT_BRANCH_TIP' } });
